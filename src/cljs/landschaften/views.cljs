...
(defn info [painting]
  (let [info-list '(:title :author :date :timeframe :type :school)]
    [:div
      (map
        (fn [datum] [:div (kv-str (find painting datum))])
        info-list)
      [:div "I'm the info div"]]))
...
(defn app []
  (let [paintings (rf/subscribe [::subs/paintings])]
    [:div
     ...
     ;; more flexibility for the on-satisfies-fn:
     ; (if-let [jpg (coerce ::specs/jpg (:jpg (first @paintings)))]
     ;   [image jpg]
     ;   NO-IMAGE-AVAILABLE)

     [:div "... Where's the picture?"]]))
