...
;; Because: keywords provide little advantage over string keys, and require that we crawl and transform 1000s+ keys
(reg-event-fx
  ::query-started
  (fn query [cofx _]
    (let [db (:db cofx)]
...
(>defn previous-painting [db]
  ...
  (let [paintings (explore-subs/get-current-painting-window (:paintings db)
...
(defn previous-painting-window [db]
  (let [windows-count (count (explore-subs/get-painting-windows db))
        ...
        window-index (if (neg? new-window-index)
                       (dec windows-count) ; i.e. highest index
                       new-window-index)]
    (assoc db :current-painting-window-index window-index)))
...
(defn next-painting-window [db]
  (let [windows-count (count (explore-subs/get-painting-windows db))
        ...
        window-index (if (< highest-index new-window-index)
                       0
                       new-window-index)]
    (assoc db :current-painting-window-index window-index)))
